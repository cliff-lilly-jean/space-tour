 ...
 return (
  <div className='moon flex flex-col items-center m-auto pl-5 pr-5'>
   ...
   <div className="avgs md:flex md:w-96 md:justify-between">
    {/* the avg distance */}
    <p className="avg-distance text-secondary text-[12px] text-center md:text-sm">AVG. DISTANCE <span className="avg-distance__value block mt-2 mb-2 text-white text-center m-auto text-[26px] md:text-[28px]">{pageData.destinations[0].distance}</span></p>
    {/* the travel time */}
    <p className="avg-travel text-secondary text-[12px] text-center md:text-sm">EST. TRAVEL TIME<span className="avg-travel__value block mt-2 mb-2 text-white text-center m-auto text-[26px] md:text-[28px]">{pageData.destinations[0].travel}</span></p>
   </div>
